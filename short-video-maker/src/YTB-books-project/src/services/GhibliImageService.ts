...
export class GhibliImageService {
   ...
   * 지브리 스타일 프롬프트 빌드
   */
  private buildGhibliPrompt(
    basePrompt: string,
    config?: GhibliStyleConfig
  ): string {
    const parts: string[] = [GHIBLI_STYLE_PREFIX];
  ...
  async generateSceneImage(
    ...
    const ghibliPrompt = this.buildGhibliPrompt(prompt, config);

    logger.info({
      sceneIndex,
      isFirstScene,
      strategy: isFirstScene ? 'GPT-4o Ghibli' : 'NanoBanana + Reference',
    ...
    try {
      ...
      if (isFirstScene && this.gptService) {
        ...
        if (result.success && result.images?.[0]) {
          ...
          logger.info({
            ...
            generator: 'gpt'
          }, '✅ GPT reference image generated and saved');
        ...
        logger.warn({ sceneIndex, error: result.error }, '⚠️ GPT failed, falling back to NanoBanana');
      }
      ...
      if (result.success && result.images?.[0]) {
        // 🔥 수정: 첫 씬이고 referenceImage가 없으면 저장 (GPT fallback 케이스)
